...
w('(`masslegalhelp/tools/snap/index.html`) and the shared decision logic. If a string is on the page,');
...
A.GOODCAUSE_CATEGORIES.forEach(cat => {
  ...
  if (cat.moreExamplesTip.length) {
    w('  "More examples" opens a hover panel here rather than linking out, built like the ABAWD');
    w('  definition in the intro. It reads:');
    cat.moreExamplesTip.forEach(x => w('    - ' + toMarkdown(x)));
  } else if (cat.moreExamplesUrl) {
    w('  More examples link: ' + cat.moreExamplesUrl);
  } else {
    w('  No "More examples" on this one.');
  }
});
blank();
