...
const SHIP = path.join(MLH, 'snap-abawd', 'index.html');

/* One build. court-forms/ was archived on 2026-07-30 along with the two earlier
 * designs, so masslegalhelp/snap-abawd/index.html is both the page reviewers look at and the
...
const PAGES = [
  { label: 'masslegalhelp/snap-abawd/index.html', dir: path.join(MLH, 'snap-abawd'), file: 'index.html', guided: false },
...
function buildContext() {
  ...
  const ctx = {
    ...
    location: { href: 'https://example.test/masslegalhelp/snap-abawd/', search: '', pathname: '/masslegalhelp/snap-abawd/' },
...
describe('the shipping build stores answers per tab only', () => {
  const src = () => inlineScript(fs.readFileSync(SHIP, 'utf8'), 'masslegalhelp/snap-abawd/index.html');
  ...
  it('uses sessionStorage and never localStorage', () => {
    ...
    assert.doesNotMatch(
      ...
      'masslegalhelp/snap-abawd/index.html touches localStorage, which outlives the tab. Answers ' +
...
describe('the top-bar Learn More link', () => {
  ...
  it('no quick-exit handler survives on either page', () => {
    for (const p of [SHIP, path.join(MLH, 'index.html')]) {
...
describe('review-only modes stay off a production host', () => {
  ...
  function on(hostname, search) {
    const { ctx, stage } = buildContext();
    ctx.location = {
      href: 'https://' + hostname + '/snap-abawd/' + (search || ''),
      hostname, search: search || '', pathname: '/snap-abawd/'
    };
    vm.createContext(ctx);
    vm.runInContext(logic, ctx, { filename: LOGIC_FILE });
    vm.runInContext(src, ctx, { filename: 'masslegalhelp/snap-abawd/index.html' });
    return { ctx, stage };
  }
...
describe('the EAEDC note renders under the health reason', () => {
  ...
  function render(answers) {
    ...
    vm.runInContext(
      ...
      ctx, { filename: 'masslegalhelp/snap-abawd/index.html' }
    );
