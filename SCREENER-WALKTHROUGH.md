# The SNAP work rules screener, start to finish

_Every word the tool says, in the order someone meets it, and then a set of worked examples showing exactly how the letter to DTA gets written._

## How to read this

_One rule covers most of it: **anything in italics is explanation and is not part of the tool.** Everything else is wording the screener really shows, and is what you would be editing._

| What you see | What it means |
| --- | --- |
| _Italic text_ | Explanation, written to walk you through. Not shown to anyone using the tool. |
| > Text in a quote box | Wording the tool shows, word for word. |
| **Bold inside a quote** | The tool shows those words in bold on the screen. |
| **A bold line, then a list** | A question, and the answers someone can choose between. |
| **← picked** | In a worked example, the answer that person chose. |
| A block in a fixed-width font | A finished letter, laid out the way DTA would receive it. |
| `__________` in a letter | A line to sign by hand, when the person did not sign on screen. |
| Version A | The tool as it works today: the person writes their own statement. |
| Version B | The proposed version: the tool writes the statement from their answers. |

_Links are clickable behind the link text in markdown preview. Every outbound URL is also listed at the end of the document for checking._

_**This document is for reading.** There is a second one, `SCREENER-COPY.md`, which lists the same wording with a short code beside each piece. Those codes are how an edit gets back to the right place in the software, so that is the one to write changes into. This one has no codes in it and is meant to be read straight through._

_Both are generated from the tool itself, so neither can quietly fall out of step with what the screener actually says._

_A note on what this tool is not. It does not send anything to DTA, it does not change anyone's SNAP case, and it cannot confirm an exemption. It tells someone what their answers suggest and gives them a letter they can sign and send. Nothing anyone types leaves their own device._

## Before anything else: the start page

> Did DTA tell you that you need to meet ABAWD Work Rules to keep getting SNAP?

> Some adults on SNAP who are **18 through 64** have to meet ABAWD work rules to get SNAP for more than 3 months. **Many adults are exempt**. Being "exempt" means that you don’t have to meet the work rules to keep getting SNAP.

> You only have to meet these rules or tell DTA you are exempt if DTA sends you a [SNAP and Work notice](https://eohhs.ehs.state.ma.us/DTA/PolicyOnline/olg%20docs/Consolidated%20Notice%20Sample.pdf) that says you have to meet the ABAWD Work Rules. DTA has to talk to you and check you for all exemptions *before* telling you that you have to meet the rules. Check your mail and [DTAConnect](https://dtaconnect.eohhs.mass.gov/) to see notices from DTA.

> Learn more about the ABAWD Work Rules and applying for SNAP [here](https://www.masslegalhelp.org/public-benefits-ssi/snap-food-benefits/snap-3-month-time-limit-abawd-work-rules).

> This short online form asks a few questions to check if you need to meet the ABAWD work rules or if you are exempt. **It takes less than 2 minutes.**

> Click here to check if the ABAWD work rules apply to you

> **Your information is private.** MLRI will not save any personal information you share on this form.

> The Department of Transitional Assistance (DTA) runs the SNAP program in Massachusetts. The SNAP ABAWD Work Rules are different from MassHealth work rules.

## The questions

> Answer any that apply to you. **Every question is optional.**

_Nothing is required, and clicking an answer a second time clears it. The questions come in four sections._

### Section 1 of 4: Your Family and Household

**Are you 18 through 64 years old?**

- Yes
- No

**Do you live with a child under 14 years old?**

If you live with a child under 14 who should be part of your [SNAP household](https://www.masslegalhelp.org/public-benefits-ssi/snap-food-benefits/34-what-snap-household-or-assistance-unit), even if they are not eligible (for example, because of immigration status or if they are a foster child), select “Yes.”

- Yes
- No

**Do you take care of a child under 6 years old?**

You do not need to be related to them, live with them, or provide care full-time.

- Yes
- No

**Do you take care of a child or adult who cannot care for themselves?**

You do not need to be related to them, live with them, or provide care full-time.

Caring for someone means you regularly do things like make food, help with daily tasks like bathing and personal care, run errands for them, and monitor their health and wellbeing. If you are an [Adult Foster Care provider](https://www.masslegalhelp.org/public-benefits-ssi/snap-food-benefits/adult-foster-care-and-snap), answer “Yes.”

- Yes
- No

**Are you pregnant?**

- Yes
- No

### Section 2 of 4: Your health, housing, and safety

**Do you have a health reason or disability that makes it hard to work at least 30 hours a week?**

It can be a physical or mental health reason, and short- or long-term.

- Yes
- No

**Do you have a regular place to sleep at night?**

Choose “No” if you are experiencing homelessness or unstable housing, like living in a shelter or couch surfing.

- Yes
- No

_Only shown when the answer to "Do you have a regular place to sleep at night?" is NO._

**Please choose all that apply:**

- I have a high school diploma (including GED or HiSet)
- I regularly see a health care provider for an ongoing illness, like a dentist, therapist, psychiatrist, or doctor for ongoing treatment
- I have had a steady job for at least 6 months in the last year
- I have been a full-time student for at least 6 months in the last year
- I have been hospitalized in the last 6 months
- None of the above

**Has a domestic violence or safety situation made it hard for you to work?**

This includes stalking, harassment, abuse, assault, or any health/safety concerns that make it hard for you to work. DTA has domestic violence specialists in each local office who can help. Find their [contact info here](https://www.mass.gov/info-details/dta-domestic-violence-services). Your answer stays private.

- Yes
- No

### Section 3 of 4: Public Benefits and Participating in Programs

**Do you get, or are you applying for TAFDC cash assistance benefits?**

- Yes
- No

**Do you get any of these disability benefits?**

Choose Other only if you receive a disability benefit or payment that is not listed above. Tell DTA the name of the benefit or payment so they can review it.

- EAEDC
- Veteran’s disability benefit
- Workers’ compensation
- Paid Family Medical Leave
- Short-term disability
- SSI or SSDI
- Other disability benefit or payment
- None of the above

**Are you participating in a substance use treatment program?**

This can be for drugs or alcohol. It does not have to be a daily program.

- Yes
- No

**Do you get, or are you applying for unemployment benefits?**

- Yes
- No

**Do you get services from any of these state agencies?**

- MassAbility (formerly Mass Rehab Commission)
- Dept. of Mental Health
- Dept. of Developmental Services
- MA Commission for the Blind
- MA Commission for Deaf and Hard of Hearing
- No

### Section 4 of 4: School, work, and background

**Are you enrolled in school half-time or more?**

This includes high school, vocational/technical school, college, or any education and training program. You can ask your school if you're unsure if you are enrolled half-time or more.

- Yes
- No

**Are you currently working for pay? Choose the option that applies to you.**

- Yes, I am making $217.50 a week or more (before taxes)
- Yes, I am working at least 14.5 hours a week at $15 or more an hour
- Yes, I am working 30 hours or more a week (I make less than minimum wage)
- None of the above

**Are you, or is your parent or grandparent, an Alaska native or a member of an American Indian, Urban Indian, or California Indian tribe?**

- Yes
- No

### One more question, for some people

_Anyone who comes out exempt never sees this. It is only asked of someone the screening has not found an exemption for, because it is about missing hours rather than about not having to meet the rules in the first place._

**Is something making it hard to work, go to school, or volunteer right now?**

These are situations where you missed work, school, or volunteering hours for one or more months because of an unexpected life event.Some examples of unreasonable employment:
 - Employer or workplace discrimination
 - The work is a risk to your health and safety
 - Work interferes with your religious observances or beliefs
 - Traveling to and from work takes over 2 hours, or, if you walk to work, the round trip commute is over 2 miles
 - Job pays less than $15/hour (minimum wage)
 - There is a strike

- Yes, my ride broke down or I have temporary transportation issues
- Yes, I have a family or personal emergency
- Yes, I’m dealing with unreasonable employment
- This question does not apply to me / I’m not sure

## What someone is told at the end

_There are three outcomes, and the wording of each is below._

### 1. You may be exempt

> You may be **exempt** and **do not need to meet the ABAWD work rules** because of these reasons:

_Followed by a checklist of the reasons that applied. Every reason the tool can name:_

- I live with a child under 14 who should be part of my SNAP case
- I have a health reason that makes it hard to work 30 or more hours a week
- I take care of a child under 6 years old
- I take care of a child or adult who cannot care for themselves
- I am pregnant
- I am experiencing or have experienced domestic violence, stalking, sexual harassment, sexual assault, or another safety situation that makes it hard to work
- I am an Alaska Native or member of a Tribe (or I am a child or grandchild of a member of a Tribe)
- I get or am applying for TAFDC cash assistance
- I get disability benefits
- I get another disability benefit or payment DTA should review
- I am participating in a substance use treatment program
- I get or am applying for unemployment benefits
- I am enrolled in school half-time or more
- I earn enough money from work to be exempt from the work rules
- I work 30 or more hours a week while earning less than minimum wage
- I do not have a regular place to sleep

_Some of them add a line about what to send DTA:_

> Send DTA proof of your income and hours, such as pay stubs or a letter.

> Tell DTA the details about your housing so they can review your exemption.

> Tell DTA the details about your disability benefit so they can review your exemption.

### 2. You may have a good reason for missing hours

> You may have a good reason for missing hours

> This includes missing work, school, or volunteer hours before or after your start date.

> Tell DTA as soon as you can if you could not meet the work rules for one or more months because of a hard life event, like:

_All three categories are listed, not only the one the person picked, because someone may recognise their situation in a different one:_

**No transportation**
A temporary loss of transportation, like a broken down car or temporary public transportation shutdown.

**Emergency**
Any family, personal crisis, or emergency situation, and/or if you need to give care or support to others.

**Employment issues**
Employer or work environment discriminates on the basis of age, sex, race, religion, ethnicity, or physical or mental disability.

### 3. You may need to meet the work rules

> You may need to meet the ABAWD work rules

> Based on your responses, you may not be exempt from the ABAWD work rules.

_Then the two ways to meet the rules. Both option headings are bold on the screen:_

> **Paid work, unpaid work, or training program for 20 hours a week (80 hours a month).**

- Examples of unpaid work can include internships or caring for family or friends who are not disabled or under age 6.
- [Find a DTA training program](https://snappathtowork.org/)

_or_

> **Community service for a set number of hours each month. The number depends on how much SNAP you get. DTA will tell you how many hours to volunteer.**

_and a reminder that a good reason for missing hours still counts. This one is a heading on the page, so it is bold there too:_

> **You may have a good reason for missing work, school, or volunteer hours.**

> Tell DTA as soon as possible if you couldn’t meet the work rules for one or more months because of an unexpected life situation like temporary transportation issues, a personal or family emergency, or employment issues.

## The letter to DTA, and the two ways of writing it

_The first two outcomes, exempt and good reason, both end with a letter the person can print, download, or email, then sign and send to DTA. Everything about that letter is the same in both versions except the middle: the part in the person's own voice explaining their situation._

_**There are two versions of that middle part, and choosing between them is the open question this document exists to help with.**_

| | Version A: the person writes it | Version B: the tool writes it |
| --- | --- | --- |
| What they see | One empty box per exemption, with a prompt above it | Two or three multiple-choice questions, then the finished sentences |
| What they type | A few sentences, in their own words | Nothing |
| Extra questions | None | 2 to 5, on one screen before the results |
| Also needed | Name, ID, signature | Name, ID, signature |
| Shipping today | Yes | No, under review |

_Version A is the tool as it stands. Version B is the proposal. Both are on the review site now so they can be compared on the thing that actually matters: whether DTA would accept a statement written by the tool._

### Version A: the person writes it

_The results screen says:_

> Tell DTA that you are exempt as soon as you can.

> To tell DTA you are exempt, you can fill in the blanks below with your results and send it to DTA.

> **Your information is private.** MLRI will not save any personal information you share on this form.

_Then a heading, and under it one empty box for each thing that needs explaining:_

> In a few sentences:

_Which boxes appear depends on why the person came out exempt. All of the prompts:_

- Explain your reasons in your own words
- Explain the health reason that makes it hard for you to work 30 or more hours a week
- Explain your caretaking responsibilities or arrangement
- Explain your caretaking arrangement for the child under 6
- Explain your work hours, pay, and what proof you can send to DTA
- Name the disability benefit or payment you receive
- Explain where you sleep and any barriers that make it hard to work
- Explain why you had to miss work, school, or volunteer hours

_The last one on that list is worth pausing on. Someone exempt for a reason that needs no explaining, such as being pregnant, still gets a box, and the only prompt the tool has for them is "Explain your reasons in your own words" over an empty space._

### Version B: the tool writes it

_Instead of a box, one extra screen before the results:_

> A few more details for your letter

> These answers write your letter for you. Every question is optional, and anything you skip is simply left out.

> Your answers stay on this device. MLRI does not see them.

_Only the questions about that person's own exemption are asked, so most people see two or three. Someone exempt for a reason that speaks for itself is asked nothing at all and goes straight to the results with a finished letter._

_Then the results screen shows what was written, so nobody signs something they have not read:_

> We wrote this from your answers. Read it over, then print or download it to send to DTA.

> **Your information is private.** MLRI will not save any personal information you share on this form.

> What your letter says:

_...the sentences, and then a way back:_

> Change my answers

_**Two rules the tool follows when it writes.** Nothing is invented: a skipped question drops its sentence rather than filling it in with something likely. And every question has one way to decline, which produces a shorter true sentence rather than a blank._

## How the letter gets written, step by step

_Six people, each shown all the way through: what they clicked, what each click added to the letter, and the letter they end up with. Every sentence below was produced by running the real tool, not written by hand._

Dates are fixed at August 1, 2026 so this document does not change on its own.

### Example 1: Someone with a health condition

_The most common exemption, and the one with the most to explain._

_**What they answered in the screening**_

- Do you have a health reason or disability that makes it hard to work at least 30 hours a week? — **Yes**

_**Which gives**_

_The exempt result, listing:_

- I have a health reason that makes it hard to work 30 or more hours a week

_**What Version B asks next, and what each answer writes**_

_The letter is built up one answer at a time. After each question, what is shown is the whole statement so far, not only the part that answer added, so you can watch it grow._

**Is this a physical health reason, a mental health reason, or both?**

- A physical health reason   **← picked**
- A mental health reason
- Both
- I would rather not say

_The letter now reads:_

> I have a physical health condition that makes it hard for me to work 30 or more hours a week.

**How long has this been going on, or how long do you expect it to last?**

- Less than 6 months
- 6 months or more   **← picked**
- I do not know

_The letter now reads:_

> I have a physical health condition that makes it hard for me to work 30 or more hours a week. It has lasted 6 months or more, or I expect it to.

**Do you see a doctor, therapist, or other provider for it?**

- Yes, regularly   **← picked**
- Yes, sometimes
- No
- I am not sure

_The letter now reads:_

> I have a physical health condition that makes it hard for me to work 30 or more hours a week. It has lasted 6 months or more, or I expect it to. I see a health care provider for it regularly, and I can ask them for a letter if you need one.

_**The letter they get**_

```
Date:               August 1, 2026
To:                 Massachusetts Department of Transitional Assistance (DTA)
From:               Jordan Rivera
Re:                 SNAP benefits, ABAWD work rules

Dear DTA,

I am writing to ask that you update my SNAP case. I believe I am exempt from the ABAWD work rules and should not have to meet them for the following reason(s):

I have a physical health condition that makes it hard for me to work 30 or more hours a week. It has lasted 6 months or more, or I expect it to. I see a health care provider for it regularly, and I can ask them for a letter if you need one.

Sincerely,

__________________________

Jordan Rivera

Date signed: August 1, 2026
```

_In Version A, the same person sees one empty box instead, under:_

- Explain the health reason that makes it hard for you to work 30 or more hours a week

### Example 2: Someone caring for a parent

_Shows how the sentence names who is cared for without asking anything identifying._

_**What they answered in the screening**_

- Do you take care of a child or adult who cannot care for themselves? — **Yes**

_**Which gives**_

_The exempt result, listing:_

- I take care of a child or adult who cannot care for themselves

_**What Version B asks next, and what each answer writes**_

_The letter is built up one answer at a time. After each question, what is shown is the whole statement so far, not only the part that answer added, so you can watch it grow._

**Who do you take care of?**

- A child
- An adult   **← picked**
- More than one person
- I would rather not say

_The letter now reads:_

> I take care of an adult who cannot care for themselves.

**How often do you provide this care?**

- Every day   **← picked**
- Most days of the week
- A few days a week
- Whenever they need me
- I am not sure

_The letter now reads:_

> I take care of an adult who cannot care for themselves. I do this every day.

**Is anyone else helping with this care?**

- I am the only one   **← picked**
- I share it with someone else
- I am not sure

_The letter now reads:_

> I take care of an adult who cannot care for themselves. I do this every day. I am the only person providing this care.

_**The letter they get**_

```
Date:               August 1, 2026
To:                 Massachusetts Department of Transitional Assistance (DTA)
From:               Alex Chen
Re:                 SNAP benefits, ABAWD work rules

Dear DTA,

I am writing to ask that you update my SNAP case. I believe I am exempt from the ABAWD work rules and should not have to meet them for the following reason(s):

I take care of an adult who cannot care for themselves. I do this every day. I am the only person providing this care.

Sincerely,

__________________________

Alex Chen

Date signed: August 1, 2026
```

_In Version A, the same person sees one empty box instead, under:_

- Explain your caretaking responsibilities or arrangement

### Example 3: Someone working part time

_The proof question matters here: DTA wants documents, and the letter should only promise the ones the person actually has._

_**What they answered in the screening**_

- Are you currently working for pay? — **Yes, I am making $217.50 a week or more (before taxes)**

_**Which gives**_

_The exempt result, listing:_

- I earn enough money from work to be exempt from the work rules

_**What Version B asks next, and what each answer writes**_

_The letter is built up one answer at a time. After each question, what is shown is the whole statement so far, not only the part that answer added, so you can watch it grow._

**About how many hours a week do you usually work?**

- Less than 10 hours
- About 10 to 19 hours
- About 20 to 29 hours   **← picked**
- 30 hours or more
- It changes week to week
- I am not sure

_The letter now reads:_

> I earn enough income to be exempt from the ABAWD work rules. I usually work about 20 to 29 hours a week.

**How many jobs do you have?**

- One job   **← picked**
- More than one job
- I am not sure

_The letter now reads:_

> I earn enough income to be exempt from the ABAWD work rules. I usually work about 20 to 29 hours a week at one job.

**What can you send DTA as proof of your work?**

- Pay stubs   **← picked**
- A letter from my employer   **← picked**
- My work schedule
- I need help getting proof
- None of these

_The letter now reads:_

> I earn enough income to be exempt from the ABAWD work rules. I usually work about 20 to 29 hours a week at one job. I can send you my pay stubs and a letter from my employer.

_**The letter they get**_

```
Date:               August 1, 2026
To:                 Massachusetts Department of Transitional Assistance (DTA)
From:               Sam Okafor
Re:                 SNAP benefits, ABAWD work rules

Dear DTA,

I am writing to ask that you update my SNAP case. I believe I am exempt from the ABAWD work rules and should not have to meet them for the following reason(s):

I earn enough income to be exempt from the ABAWD work rules. I usually work about 20 to 29 hours a week at one job. I can send you my pay stubs and a letter from my employer.

Sincerely,

__________________________

Sam Okafor

Date signed: August 1, 2026
```

_In Version A, the same person sees one empty box instead, under:_

- Explain your work hours, pay, and what proof you can send to DTA

### Example 4: Someone with no regular place to sleep

_Several answers at once, composed as separate sentences rather than a list._

_**What they answered in the screening**_

- Do you have a regular place to sleep at night? — **No**
- Please choose all that apply: — **None of the above**

_**Which gives**_

_The exempt result, listing:_

- I do not have a regular place to sleep

_**What Version B asks next, and what each answer writes**_

_The letter is built up one answer at a time. After each question, what is shown is the whole statement so far, not only the part that answer added, so you can watch it grow._

**Where do you usually sleep?**

- In a shelter   **← picked**
- Outside, or in a car
- At other people’s homes
- In a motel or hotel
- Somewhere different from night to night
- I would rather not say

_The letter now reads:_

> I do not have a regular place to sleep. I usually sleep in a shelter. Please review my situation to decide whether I am unable to work under the ABAWD screening.

**What makes it hard for you to work?**

- I have no address or phone to give an employer   **← picked**
- I have no safe place to keep my things
- I have no reliable way to get to a job   **← picked**
- I have health problems
- I have to move often
- I do not feel safe
- None of these

_The letter now reads:_

> I do not have a regular place to sleep. I usually sleep in a shelter. This makes it hard for me to work. I have no address or phone to give an employer. I have no reliable way to get to a job. Please review my situation to decide whether I am unable to work under the ABAWD screening.

_**The letter they get**_

```
Date:               August 1, 2026
To:                 Massachusetts Department of Transitional Assistance (DTA)
From:               Riley Santos
Re:                 SNAP benefits, ABAWD work rules

Dear DTA,

I am writing to ask that you update my SNAP case. I believe I am exempt from the ABAWD work rules and should not have to meet them for the following reason(s):

I do not have a regular place to sleep. I usually sleep in a shelter. This makes it hard for me to work. I have no address or phone to give an employer. I have no reliable way to get to a job. Please review my situation to decide whether I am unable to work under the ABAWD screening.

Sincerely,

__________________________

Riley Santos

Date signed: August 1, 2026
```

_In Version A, the same person sees one empty box instead, under:_

- Explain where you sleep and any barriers that make it hard to work

### Example 5: Someone who missed hours because their car broke down

_The only case where the tool names dates. Note that the questions say "last month", never a month name, so this wording does not change over time._

_**What they answered in the screening**_

- Is something making it hard to work, go to school, or volunteer right now? — **Yes, my ride broke down or I have temporary transportation issues**

_**Which gives**_

_The good-reason result:_

- No transportation — a temporary loss of transportation, like a broken down car or temporary public transportation shutdown.

_**What Version B asks next, and what each answer writes**_

_The letter is built up one answer at a time. After each question, what is shown is the whole statement so far, not only the part that answer added, so you can watch it grow._

**What happened?**

- My car broke down   **← picked**
- I lost my ride
- Public transportation was not running
- I could not afford to get there
- Something else

_The letter now reads:_

> My car broke down and I had no other way to get there.

**Which months did this affect?**

- This month   **← picked**
- Last month   **← picked**
- The month before that
- More than three months
- I am not sure

_The letter now reads:_

> My car broke down and I had no other way to get there. I missed hours in July and August 2026.

**Is this still going on?**

- Yes, it is still going on   **← picked**
- No, it is over now
- I am not sure

_The letter now reads:_

> My car broke down and I had no other way to get there. I missed hours in July and August 2026. This is still going on.

_**The letter they get**_

```
Date:               August 1, 2026
To:                 Massachusetts Department of Transitional Assistance (DTA)
From:               Dana Whitfield
Re:                 SNAP benefits, ABAWD work rules

Dear DTA,

I am writing to explain why I could not meet the ABAWD work rules for one or more months. My good-cause reason is:

No transportation — a temporary loss of transportation, like a broken down car or temporary public transportation shutdown.

My car broke down and I had no other way to get there. I missed hours in July and August 2026. This is still going on.

Sincerely,

__________________________

Dana Whitfield

Date signed: August 1, 2026
```

_In Version A, the same person sees one empty box instead, under:_

- Explain why you had to miss work, school, or volunteer hours

### Example 6: Someone who is pregnant

_Nothing needs explaining, so nothing is asked. This is the clearest difference between the two versions: in Version A this person gets an empty box under "Explain your reasons in your own words"._

_**What they answered in the screening**_

- Are you pregnant? — **Yes**

_**Which gives**_

_The exempt result, listing:_

- I am pregnant

_**What Version B asks next**_

_Nothing. There is nothing here that needs explaining to DTA, so the letter is already finished and this person goes straight to it._

_**The letter they get**_

```
Date:               August 1, 2026
To:                 Massachusetts Department of Transitional Assistance (DTA)
From:               Casey Brooks
Re:                 SNAP benefits, ABAWD work rules

Dear DTA,

I am writing to ask that you update my SNAP case. I believe I am exempt from the ABAWD work rules and should not have to meet them for the following reason(s):

  - I am pregnant

Sincerely,

__________________________

Casey Brooks

Date signed: August 1, 2026
```

_In Version A, the same person sees one empty box instead, under:_

- Explain your reasons in your own words

## Where the multiple-choice version is worse

_One case is worth deciding on deliberately rather than discovering later._

_When someone says they get a disability benefit that was not on the screening list, the write-in version asks them to name it, and can take any answer. A multiple-choice list cannot hold every disability payment there is, so it offers the common ones and then has to fall back. Naming one:_

> I receive MassHealth based on a disability determination. Please review it as part of my exemption screening.

_and not naming one:_

> I receive a disability benefit or payment that was not on the list. I will bring the paperwork so you can review it.

_The second is weaker than what a written answer could have said. The choices offered before it gets there:_

- MassHealth based on a disability determination
- Private or employer disability insurance
- A VA pension
- Railroad Retirement disability benefits
- A Tribal disability payment
- A disability payment from another state
- Something not on this list

_If DTA would rather have the name in every case, this is the one place to keep a box._

## What happens when someone skips the questions

_Every question in the tool is optional, including these, so the letter has to hold up when they are left alone. It does this by dropping sentences, never by guessing._

_Someone who answers the health question in the screening and then skips all three follow-ups still gets a true letter:_

> I have a health condition that makes it hard for me to work 30 or more hours a week.

_Answering only the first of the three:_

> I have a physical health condition that makes it hard for me to work 30 or more hours a week.

_And all three:_

> I have a physical health condition that makes it hard for me to work 30 or more hours a week. It has lasted 6 months or more, or I expect it to. I see a health care provider for it regularly, and I can ask them for a letter if you need one.

_The same holds for choosing "I would rather not say", which is offered on the questions where someone might not want to answer. It produces the shorter sentence, not a gap._

## Every sentence the tool can write

_The full set, for anyone who wants to read the wording without walking through an example to reach it. Each is shown with every question answered, which is its longest form._

**A health reason**

> I have both a physical and a mental health condition that makes it hard for me to work 30 or more hours a week. It has lasted 6 months or more, or I expect it to. I see a health care provider for it regularly, and I can ask them for a letter if you need one.

**Caring for someone**

> I take care of more than one person who cannot care for themselves. I do this most days of the week. I share this care with someone else.

**Caring for a child under 6**

> I take care of a child under 6 years old. The child lives with me. I care for them every day.

**Working**

> I earn enough income to be exempt from the ABAWD work rules. I usually work 30 or more hours a week at more than one job. I can send you my pay stubs, a letter from my employer, and my work schedule.

**Working, but needs help getting proof**

> I work 30 or more hours a week while earning less than minimum wage. I need help getting proof of my work hours and pay.

**Another disability benefit**

> I receive private or employer disability insurance. Please review it as part of my exemption screening.

**No regular place to sleep**

> I do not have a regular place to sleep. I usually sleep outside, or in a car. This makes it hard for me to work. I have no address or phone to give an employer. I have no safe place to keep my things. I have no reliable way to get to a job. I have health problems. I have to move often. I do not feel safe. Please review my situation to decide whether I am unable to work under the ABAWD screening.

**A good reason: transportation**

> Public transportation was not running when I needed it. I missed hours in July 2026. This has since been resolved.

**A good reason: an emergency**

> I had to take care of someone who needed me. I missed hours in August 2026. This is still going on.

**A good reason: a job situation**

> My employer treated me unfairly because of who I am. I missed hours for more than three months. This is still going on.

## What the person still has to do themselves

_Neither version can write these, and neither tries to._

> Your name

> Client / Agency ID (if you have one)

> Signature

> If you cannot sign here, leave it blank. The printed page has a line to sign by hand.

_The signature is drawn with a finger or a mouse, which means someone using a keyboard, a switch, or a screen reader cannot sign in the browser. When the box is left empty the printed letter has a ruled line to sign by hand instead, which is what the last sentence above is telling them._

## The numbers behind the work question

_Not wording, but they decide who comes out exempt and they appear in the questions. Last checked November 2025, and the MLRI article on MassLegalHelp was reviewed in February 2026, so the article is newer than the tool._

| What | Value |
| --- | --- |
| Weekly earnings that make someone exempt | $217.5 |
| Massachusetts minimum wage used | $15 an hour |
| Hours a week at minimum wage that make someone exempt | 14.5 |
| Hours a week that count as meeting the rules | 30 |

## Where the tool sends people

_Every outbound link, worth checking that each still goes somewhere useful._

- https://www.masslegalhelp.org/public-benefits-ssi/snap-food-benefits/snap-3-month-time-limit-abawd-work-rules
- https://www.masslegalhelp.org/public-benefits-ssi/snap-food-benefits/snap-3-month-time-limit-abawd-work-rules#:~:text=What%20if%20I%20had%20a%20good%20reason%20for%20not%20meeting%20the%20Work%20Rules%3F
- https://eohhs.ehs.state.ma.us/DTA/PolicyOnline/olg%20docs/Consolidated%20Notice%20Sample.pdf
- https://dtaconnect.eohhs.mass.gov/
- https://www.masslegalhelp.org/public-benefits-ssi/snap-food-benefits/34-what-snap-household-or-assistance-unit
- https://www.masslegalhelp.org/public-benefits-ssi/snap-food-benefits/adult-foster-care-and-snap
- https://www.mass.gov/info-details/dta-domestic-violence-services
- https://www.mass.gov/doc/snap-work-rules-exemption-self-declaration-form/download
- https://www.masslegalservices.org/system/files/library/Advocacy%20Tips_%20Reaching%20a%20DTA%20worker.pdf
- https://www.mass.gov/orgs/department-of-transitional-assistance/locations
- https://www.mass.gov/how-to/supplemental-nutrition-assistance-program-snap-formerly-known-as-food-stamps
- https://www.masslegalhelp.org/sites/default/files/2025-11/Terminations%20OB3%20KYR%20SNAP%20ABAWDs%20Flyers%20.pdf
- https://snappathtowork.org/
- info@masslegalservices.org

---

_Generated from the screener itself by `scripts/copy-walkthrough.js`. Re-running it after a change updates this document, and the build fails if that has not happened, so nothing here can quietly describe a version of the tool that no longer exists._
